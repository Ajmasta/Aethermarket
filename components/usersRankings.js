...
const UsersRankings = ({ collection, name }) => {
      ...
      <div className={styles.tableUserContainer}>
        ...
        <div className={styles.tableUserRow}>
          ...
          <p className={styles.tableUserCell}>
            {parseFloat(users[userIndex][1] * 10000000).toFixed(2)}
          </p>

          <p className={styles.tableUserCell}>{users[userIndex][2].length}</p>
        </div>
      </div>
    );
  };
    ...
    return (
      <div className={styles.tableContainer}>
        <div className={`${styles.tableRow} ${styles.tableFirstRow}`}>
          ...
          <div className={`${styles.tableCell} ${styles.tableCellHelp}`}>
            Points{"    "}
            <HelpOutlineIcon
            ...
            />
            <div className={help ? styles.helpBox : styles.hidden}>
              <p>
                Points are calculated based on the rank of each NFT. These
                calculations heavily favor ranks over quantity.{" "}
              </p>
            </div>
          </div>
          <p className={styles.tableCell}># of Assets</p>
        </div>
        {users.map((user, i) => (
          <>
            <div
            ...
            >
              ...
              <p className={styles.tableCell}>
                {parseFloat((user[1] * 10000000).toFixed(2))}
              </p>

              <p className={styles.tableCell}>{user[2].length}</p>
            </div>
  ...
  const createUsersChart = (userArray) => {
    ...
    const users = usersRankings[collection].userRanks.slice(0, 200);
    ...
    <>
      {usersCountArray ? (
        <div className={styles.mainContainer}>
          ...
          {activeTab === "table" ? (
            <>
              {createUsersTable(usersRankings, numberOfItems)}
            ...
            <>
              <div className={styles.chartContainer}>
                {createUsersChart(usersRankings)}
              </div>
            </>
          )}
        </div>
