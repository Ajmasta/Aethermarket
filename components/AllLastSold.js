...
const AllLastSold = ({data}) => {
    data.result.sort((result, result2)=> result.updated_timestamp < result2.updated_timestamp ? 1:-1)
...
<div className={styles.allListingsContainer}>
    ...
    <button onClick={()=>setDataCut(data.result.slice(0,dataCut.length+100))}>More Results</button>
    </div>)
