...
import { fillOrder } from "./functions/ImxFunctions";
import {
    RecoilRoot,
    atom,
    selector,
...
console.log(listingData)
    ...
    const getCurrentPricesChart = (data) =>{
        ...
        const sellingPrice = selling.map((result,i)=>{
            console.log(result.sell.data.token_id === listingData.sell.data.token_id)
            result.sell.data.token_id === listingData.sell.data.token_id? 
            chartData.datasets[0].data.push({x:i+1,y:result.buy.data.quantity/(10**18)}):
            i<= 10? chartData.datasets[1].data.push({x:i+1,y:result.buy.data.quantity/(10**18)}):
            chartData.datasets[2].data.push({x:i+1,y:result.buy.data.quantity/(10**18)})
        ...
        const averagePrice = sellingPrice.reduce((a,b)=>a+b)/sellingPrice.length
    ...
    const createListingsTable = (array,numberOfItems) =>{
        let selling = isSimilarSold? array.similarListings.result : array.similarCollection.result
    ...
    return (<>
        ...
        <div className={styles.mainContainer}>
            ...
            <div className={styles.leftContainer} >
                ...
                "" } </div>
                {listingData.status==="active"? checkOwnerShip()?<button onClick={()=>fillOrder(listingData)} className={styles.buyButton}>Sell </button>:<button onClick={()=>fillOrder(listingData)} className={styles.buyButton}>Buy </button>:
                <div> 
