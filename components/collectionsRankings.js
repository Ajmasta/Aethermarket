    ...
    for (let element in object) {
      if (collections && collections.length > 0) {
        ...
        array.push([
          element,
          object[element],
          collection[0].collection_image_url,
          collection[0].address,
          collection[0].icon_url,
          collectionIconUrl,
        ]);
      }
    }
      ...
      <>
        <div className={styles.tableContainer}>
          {array.slice(0, 15).map((element, i) => {
            let changeValue =
              activeTab === "day"
                ? orderData["changeDay"][element[0]]?.toFixed(2)
                : orderData["changeWeek"][element[0]]?.toFixed(2);

            return (
              <Link
                key={`${i}collec`}
                className={styles.rowContainer}
                href={`/collections/${element[3]}`}
              >
                <a className={styles.rowContainer}>
                  ...
                  {activeTab !== "all" ? (
                    <p
                      className={
                        changeValue > 0
                          ? styles.volumeChangeP
                          : changeValue < 0
                          ? styles.volumeChangeN
                          : styles.volumeChange
                      }
                    >
    ...
    <div className={styles.mainContainer}>
      ...
      <div className={styles.tabsContainer}>
        <p
          onClick={() => {
            setActiveTab("");
            setTimeout(() => setActiveTab("day"), 200);
          }}
          className={
            activeTab === "day"
        ...
        <p
          onClick={() => {
            setActiveTab("");
            setTimeout(() => setActiveTab("week"), 200);
          }}
          className={
            activeTab === "week"
              ? `${styles.tab} ${styles.activeTab}`
